...
DB_PATH="$ROOT/.orchestration/metrics.db"
LOG_PATH="$ROOT/.orchestration/tasks.jsonl"
...
USAGE = """Usage:
  orch-metrics-db.sh import
  orch-metrics-db.sh import --full
  orch-metrics-db.sh trends
  orch-metrics-db.sh trends --days 30
  orch-metrics-db.sh compare <batch-a> <batch-b>
  orch-metrics-db.sh slow [--top N]
  orch-metrics-db.sh rollup
  orch-metrics-db.sh status"""
SPARK = "▁▂▃▄▅▆▇█"
