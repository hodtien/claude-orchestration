#!/usr/bin/env bash
# _dashboard/slo.sh — SLO performance report
# Sourced by orch-dashboard.sh. Supports --batch --agent flags.
set -euo pipefail
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --help|-h) echo "Usage: slo [--batch <batch-id>] [--agent <agent>]"; exit 0 ;;
    *) echo "Unknown arg: $1" >&2; echo "Usage: slo [--batch <batch-id>] [--agent <agent>]" >&2; exit 1 ;;
  esac
done
...
if not rows:
    print("No matching SLO-enabled completion events with known duration_s."); sys.exit(0)
