#!/usr/bin/env bash
# orch-report.sh — Generate self-contained HTML orchestration dashboard
set -euo pipefail
...
Usage:
  orch-report.sh
  orch-report.sh --output <path>
  orch-report.sh --open
  orch-report.sh --last <N>
EOF
}
while [ $# -gt 0 ]; do
  case "$1" in
...
parse_ts=lambda raw: (None if not raw else (lambda t: t.astimezone(dt.timezone.utc))(dt.datetime.fromisoformat(str(raw).replace("Z","+00:00")).replace(tzinfo=dt.timezone.utc) if dt.datetime.fromisoformat(str(raw).replace("Z","+00:00")).tzinfo is None else dt.datetime.fromisoformat(str(raw).replace("Z","+00:00"))))
def ts(raw):
...
parts=[
    ...
    "<head>",
    "<meta charset='utf-8'>",
    f"<title>Orchestration Report - {h(project_root.name)}</title>",
    f"<style>{style}</style>",
    "</head>",
    "<body>",
]
